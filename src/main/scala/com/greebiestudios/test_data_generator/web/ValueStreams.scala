...
object ValueStreams
  ...
  def getChiSquared(
    ...
    val generatorChi: Generator[Double] =
      GeneratorImpl(0, 0, dof, gen = GeneratorType.ChiSquared)
  ...
  def getChiSquaredLong(
      dof: Int,
      seed: Option[Int] = None
  ): Source[NumberValue, Cancellable] = {
    val generatorChi: Generator[Long] = GeneratorLong(
      GeneratorImpl(0, 0, dof, gen = GeneratorType.ChiSquared)
    )
    generatorChi.seed = seed
    val chiSquared: Source[NumberValue, Cancellable] =
  ...
  def getBeta(
    ...
    val generatorBeta: Generator[Double] =
      GeneratorImpl(valueA, valueB, 10, gen = GeneratorType.Beta)
  ...
  def getBetaLong(
    ...
    val generatorBetaLong: Generator[Long] = GeneratorLong(
      GeneratorImpl(valueA, valueB, 10, gen = GeneratorType.Beta)
    )
  ...
  def getExponential(
    ...
    val generatorExponential: Generator[Double] =
      GeneratorImpl(valueA, 0, 10, gen = GeneratorType.Exponential)
  ...
  def getExponentialLong(
      valueA: Double,
      seed: Option[Int] = None
  ): Source[NumberValue, Cancellable] = {
    val generatorExponential: Generator[Long] = GeneratorLong(
      GeneratorImpl(valueA, 0, 10, gen = GeneratorType.Exponential)
    )
    generatorExponential.seed = seed
    val exponential: Source[NumberValue, Cancellable] =
